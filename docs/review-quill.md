...
Full setup, configuration, and troubleshooting reference for `review-quill`: the strict reviewer that checks the exact PR head from a real checkout, sends stale or misaligned work back through ordinary GitHub reviews, and carries approval forward only when the patch identity is unchanged. For the high-level pitch, see the [package README](../packages/review-quill/README.md). For the background story, see [review-quill: a strict reviewer for your coding agent](https://blog.krasnoperov.me/posts/review-quill). For design rationale, see [design-docs/review-quill.md](./design-docs/review-quill.md).
