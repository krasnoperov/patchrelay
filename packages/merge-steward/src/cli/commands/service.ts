...
export async function handleService(parsed: ParsedArgs, stdout: Output, runCommand: CommandRunner): Promise<number> {
  ...
  if (subcommand === "restart") {
    ...
    writeOutput(
      ...
      [
        ...
        restart.ok ? `Restarted ${UNIT_NAME}` : `Restart failed: ${restart.error}`,
      ].join("\n") + "\n",
    );
    return daemonReload.ok && restart.ok ? 0 : 1;
  }
  ...
  if (subcommand === "logs") {
    ...
    const result = await runCommand("sudo", [
      "journalctl", "-u", UNIT_NAME, "-n", String(lines), "--no-pager", "-o", "short-iso",
    ]);
    if (result.exitCode !== 0) {
      throw new Error(formatCommandFailure(result, `Unable to read logs for ${UNIT_NAME}.`));
    }
    if (parsed.flags.get("json") === true) {
      const logs = result.stdout.split(/\r?\n/).filter(Boolean);
      writeOutput(stdout, formatJson({ unit: UNIT_NAME, lines, logs }));
      return 0;
    }
