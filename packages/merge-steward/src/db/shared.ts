/**
 * SQLite wrapper for merge-steward. Copied from PatchRelay's src/db/shared.ts
 * to avoid cross-package imports. Uses node:sqlite (Node 24+).
 */
...
export class SqliteConnection implements DatabaseConnection {
  ...
  transaction<T>(fn: () => T): () => T {
    return () => {
      const savepoint = `steward_txn_${this.savepointId++}`;
