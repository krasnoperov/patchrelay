  ...
  const app = await buildMultiRepoHttpServer({
    ...
    githubAdmin: {
      ...
      async discoverRepoSettings(params) {
        ...
        const discoveryOptions = params.baseBranch ? { baseBranch: params.baseBranch } : {};
        return await discoverRepoSettings(githubAuth.credentials, params.repoFullName, discoveryOptions);
      },
