...
import { computeDashboardLayout, pickVisibleParts, renderListLines } from "../src/watch/compact-layout.ts";
...
function renderedRows(totalRows: number, selectedRepoFullName: string): string[] {
  const model = modelWithFourRepos();
  const layout = computeDashboardLayout(totalRows, false);
  return renderListLines({
    model,
...
test("review-quill dashboard hides footer chrome on very small screens", () => {
...
test("review-quill dashboard restores help after the body keeps enough rows", () => {
...
test("review-quill repo visibility keeps the selected repo with a one-row body", () => {
...
test("review-quill repo visibility reserves overflow indicators after the selected row", () => {
...
test("review-quill tiny dashboard renders exact rows while moving down and up", () => {
  const originalNow = Date.now;
  Date.now = () => NOW;
  try {
    const model = modelWithFourRepos();
    const repo1 = "owner/repo-1";
