...
test("summarizeReviewBody preserves an oversized verdict line without truncation", () => {
  const verdict = `**Verdict: ${"x".repeat(2_000)}`;
  const excerpt = summarizeReviewBody(verdict);

  assert.equal(excerpt, verdict);
});
