...
- Apply a proportionality test before blocking: request changes only when the evidence shows that delaying the merge is preferable to accepting the risk now. A blocker must affect a likely near-term workflow with material impact, present a credible security, privacy, or irreversible data-loss risk, or violate explicit repository policy.
- Calibrate likelihood and impact from repository evidence rather than assumed scale or future requirements. Rare, reversible concerns and optional hardening are nits at most; prefer shipping a coherent primary flow.
