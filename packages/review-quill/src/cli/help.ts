...
function rootHelpText(): string {
  return [
    ...
    "  repo attach <owner/repo> [--base-branch <branch>] [--wait-for-green-checks [true|false]] [--required-check <checks>] [--review-doc <paths>] [--refresh] [--json]",
    ...
    "  attempts [<repo>] [<pr-number>] [--repo <id>] [--pr <num>] [--cwd <path>] [--json]",
  "                                                          Show recorded review attempts for one pull request",
  "  transcript [<repo>] [<pr-number>] [--repo <id>] [--pr <num>] [--cwd <path>] [--attempt <id>] [--json]",
  "                                                          Show the full visible Codex thread for one recorded review attempt",
  ...
  "  status [--config <path>] [--json]                      Print a one-time dashboard snapshot",
    "",
    "Resolution:",
    "  --repo and --pr (or positional <repo> <pr>) are optional when you",
    "  run the command inside a git checkout. Without them, review-quill",
    "  reads `origin`'s remote URL, matches it to an attached repoId, and",
    "  uses `gh pr view` to find the PR for the current branch. Pass",
    "  --cwd <path> to resolve from a specific directory.",
    "",
...
function repoHelpText(): string {
  return [
    ...
    "  --wait-for-green-checks      Wait for required checks before reviewing (default: false)",
