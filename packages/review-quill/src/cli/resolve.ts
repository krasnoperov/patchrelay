...
import { listRepoConfigs } from "../cli-system.ts";
import type { ParsedArgs, HelpTopic } from "./args.ts";
...
function attachHint(ref: string): string {
  ...
  const attachedHint = attached.length > 0
    ...
    : "";
  return `Repo ${ref} is not attached to review-quill. Run \`review-quill repo attach ${ref}\` first.${attachedHint}`;
}
