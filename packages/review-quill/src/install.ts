...
export async function upsertRepoConfig(options: {
  ...
  const repo: ReviewQuillRepositoryConfig = {
    ...
    reviewDocs: [...new Set((options.reviewDocs ?? (Array.isArray(existing?.reviewDocs) ? existing.reviewDocs as string[] : ["REVIEW_WORKFLOW.md"])).map((entry) => entry.trim()).filter(Boolean))],
