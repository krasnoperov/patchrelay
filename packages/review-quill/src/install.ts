...
export async function upsertRepoConfig(options: {
  ...
  const repo: ReviewQuillRepositoryConfig = {
    ...
    reviewDocs: [...new Set((options.reviewDocs ?? (Array.isArray(existing?.reviewDocs) ? existing.reviewDocs as string[] : ["REVIEW_WORKFLOW.md", "AGENTS.md"])).map((entry) => entry.trim()).filter(Boolean))],
    ...
    patchBodyBudgetTokens: options.patchBodyBudgetTokens ?? (typeof existing?.patchBodyBudgetTokens === "number" ? existing.patchBodyBudgetTokens : DEFAULT_PATCH_BODY_BUDGET_TOKENS),
  };
