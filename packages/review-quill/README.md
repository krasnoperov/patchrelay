...
Review Quill uses Codex's dedicated `review/start`, then a schema-constrained `turn/start` only to normalize the completed review. The native reviewer receives stable developer instructions separately from PR metadata and prior-review evidence. Review Quill validates and publishes the normalized verdict; Codex never posts it directly. The earlier single structured-turn mechanism is no longer supported.
