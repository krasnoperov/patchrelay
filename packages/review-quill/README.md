...
4. Relies on Codex's automatically loaded `AGENTS.md` chain and points it to configured review guidance (`REVIEW_WORKFLOW.md` by default) plus local Markdown docs explicitly referenced by the PR title/body.
...
Review Quill requires native structured verdict output from the current Codex app-server. The default `codex.reviewMode: "structured-turn"` keeps the established single-turn path. Set `codex.reviewMode: "native-two-pass"` to run Codex's dedicated `review/start` first, then use a schema-constrained `turn/start` to complete coverage beyond the native finding set and normalize the full review. The native reviewer receives stable developer instructions separately from PR metadata and prior-review evidence. Review Quill validates and publishes the normalized verdict; Codex never posts it directly.

Review threads start fresh by default. Set `codex.forkPriorReviewThread` to `true` to let a newer PR head fork the immediately preceding completed review thread when its live terminal Codex thread matches. The current metadata snapshot is still frozen against concurrent edits. Follow-up prompts receive both patch-series bases and use `git range-diff` to distinguish rebases and repair-only commits from already-reviewed patch-equivalent code; they preserve prior dispositions unless the newer patch or base materially changes that code or its contract. Fresh and follow-up prompts contain the changed-file inventory and immutable diff commands; Codex inspects patches and surrounding code in the checkout instead of receiving duplicated patch bodies. Set the option back to `false` for an immediate rollback to always-fresh review threads.
...
`pr status`, `attempts`, `transcript`, and `transcript-source` auto-resolve `--repo` and `--pr` from the current git checkout. `pr status` supports `--wait --timeout <s> --poll <s>` for blocking until a review attempt terminates. `--wait` continues through failed attempts that the service will retry automatically; plain status reports those attempts as retryable and points to service logs if recovery does not advance. Exit codes:
